...
**All methods are O(1) or O(R)** where R = replication factor (typically 3-5). No network calls, no disk I/O - pure in-memory operations!
...
**Time Complexity:** O(R) where R = replication factor (typically 3-5)  
**Network Calls:** 0
...
**Time Complexity:** O(R) - Loop through replicas  
**Network Calls:** 0
